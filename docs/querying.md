...
As the offset grows, the database still has to find and skip the preceding rows. Deep pages can therefore become progressively slower. Inserts and deletes between requests can also cause rows to be repeated or skipped. Prefer cursor-based pagination for large or frequently changing result sets.
...
For a non-unique sort column, the cursor must include every value needed to reproduce the order. The primary key acts as a tie-breaker in this example:
...
Encode cursor values before returning them from a public API, and validate decoded values before using them. To paginate backwards, reverse both the comparison operators and the sort directions, then reverse the returned page. Cursor pagination does not support jumping directly to an arbitrary page number.

Pagination does not create a database snapshot. If an ordered value changes between requests, a row can still move across the cursor. Prefer immutable ordering columns when exact traversal matters, or use the database's transaction and snapshot facilities.

### Window-Function Pagination

Window functions can assign a row number to a deterministically ordered result and are useful for reports or complex ordering where an exact row range is required. Leoric does not currently provide a database-independent window-function builder, so use a parameterized raw query and adapt the SQL to the selected database:
...
Window-function support and optimization vary by database version. This approach may still rank a large part of the result set, so benchmark it against realistic data before using it for deep pagination.
