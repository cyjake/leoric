...
To better optimize the type declarations in models, we introduced TypeScript decorators. Decorators are now fully supported and provide a cleaner way to define model attributes and associations.

#### `@Column()`

The `@Column()` decorator replaces the static `attributes` definition. It supports automatic type inference from TypeScript types, as well as explicit type overrides:
...
When the `type` option is omitted, `@Column()` infers the data type from the TypeScript type:

| TypeScript Type | Inferred Data Type     |
|-----------------|------------------------|
| `bigint`        | `BIGINT`               |
| `number`        | `INTEGER`              |
...
Association decorators replace the `static initialize()` method for defining relationships:
...
If the foreign key does not follow the naming convention, specify it explicitly:
...
For many-to-many associations, use the `through` option with `@HasMany()`:
...
To use decorators, ensure the following options are set in `tsconfig.json`:
...
For full details, see the [TypeScript Support]({{ '/types' | relative_url }}) documentation.
