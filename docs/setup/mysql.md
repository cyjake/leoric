...
Idle MySQL connections are destroyed after this many milliseconds. The timeout
starts when a connection is released back to the pool and is cancelled if the
connection is acquired again, so checked-out connections are never recycled.
...
This option works with both `mysql` and `mysql2` clients.
