...
module.exports = {
  ...
  updateOnDuplicate(opts = {}) {
    ...
    if (Array.isArray(opts.updateOnDuplicate) && opts.updateOnDuplicate.length) {
      attrStr = opts.updateOnDuplicate.map((field) => {
        const attribute = escapeId(field);
        return `${attribute}=VALUES(${attribute})`;
      }).join(',');
    } else {
      attrStr = attributes.map((attribute) => `${attribute.columnName}=VALUES(${attribute.columnName})`).join(',');
    }
    return `ON DUPLICATE KEY UPDATE ${attrStr}`;
  }
};
