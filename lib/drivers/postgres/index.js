...
module.exports = class PostgresDriver extends AbstractDriver {
  ...
  async querySchemaInfo(database, tables) {
    ...
    const text = heresql(`
      SELECT table_name, column_name,  data_type, is_nullable, column_default, character_octet_length
    ...
    for (const row of rows) {
      ...
      const result = Object.keys(row).reduce((obj, entry) => {
        obj[entry.toLowerCase()] = row[entry];
        return obj;
      }, {});
      const tableName = result.table_name;
      const columns = schema[tableName] || (schema[tableName] = []);
      let { data_type: dataType, character_octet_length: maxLength } = result;
      ...
      columns.push({
        ...
        dataType,
        allowNull: result.is_nullable !== 'NO',
      });
    }
  ...
  async alterTable(table, changes) {
    ...
    const actions = Object.keys(changes).reduce((result, name) => {
      const { exists, ...definition } = changes[name];
      const columnName = definition.columnName || snakeCase(name);

      if (exists) {
        result.push(...formatAlterColumns(columnName, definition));
      } else {
        result.push(formatAddColumn(columnName, definition));
      }
      return result;
    }, []);
