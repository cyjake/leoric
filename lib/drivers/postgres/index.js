...
function formatColumnDefinition(definition) {
  ...
  const dataType = columnType || (type ? type.toSqlString() : definition.dataType);
  const chunks = [ dataType === 'DATETIME' ? 'TIMESTAMP' : dataType  ];

  if (primaryKey || definition.autoIncrement) chunks[0] = 'SERIAL';
...
module.exports = class PostgresDriver extends AbstractDriver {
  ...
  formatColumnDefinition(definition) {
    return formatColumnDefinition(definition);
  }
