...
module.exports = class AbstractDriver {
  ...
  formatColumnDefinition(definition) {
    ...
    if (primaryKey || definition.autoIncrement) chunks.push('AUTO_INCREMENT');
  ...
  getDefinitions(attributes) {
    ...
    return Object.keys(attributes).map(name => {
      ...
      return { name, columnName: snakeCase(name), ...definition };
    });
  }

  async createTable(table, attributes) {
    const { escapeId, formatColumnDefinition } = this;
