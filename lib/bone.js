...
function compare(attributes, columns) {
  ...
  for (const name in attributes) {
    ...
    if (!definition.equals(columnMap[columnName])) {
      console.log(definition, columnMap[columnName])
      diff[name] = {
...
class Bone {
   ...
   */
  static load(columns = []) {
    const { Definition } = this.driver;
    const { attributes, options } = this;
    const attributeMap = {};

    for (const name of Object.keys(attributes)) {
      const definition = new Definition(name, attributes[name], options.define);
      attributeMap[definition.columnName] = definition;
  ...
  static cast(value, type) {
    ...
    switch (type) {
      ...
      // node-sqlite3 doesn't handle TINYINT(1) as BOOLEAN well
      case Boolean:
