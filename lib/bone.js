...
class Bone {
  ...
  static load(columns = []) {
    ...
    Object.keys(attributes).map(name => transformedAttrs[name] = new Attribute(name, attributes[name]));

    Object.defineProperties(this, looseReadonly({
      columns,
      attributeMap,
      synchronized: Object.keys(compare(transformedAttrs, columns)).length === 0,
    }));
  }
