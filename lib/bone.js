...
class Bone {
  ...
  async save() {
    ...
    if (this[primaryKey] == null) {
      await this.create({ hooks: false });
    } else if (this.attributeChanged(primaryKey)) {
      await this.upsert({ hooks: false });
    } else {
      await this.update(null, { hooks: false }); // should not run hooks
    }
    return this;
  }
  ...
  update(opts, options) {
    ...
    return Bone.update.call(Model, where, changes, {
      ...options,
   ...
   */
  async remove(forceDelete, options) {
    const Model = this.constructor;
  ...
  static init(attributes, opts = {}, descriptors = {}) {
    ...
    Object.keys(descriptors).map(key => {
      ...
      customDescriptors[key] = descriptor;
    });
