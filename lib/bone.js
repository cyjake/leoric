...
/**
 * Rename legacy timestamps to their fancier versions
 * @param {Bone} model
 */
function renameTimestamps(model) {
  const { schema } = model;
  const timestamps = {
    gmtCreate: 'createdAt',
  ...
  for (const name in timestamps) {
    const newName = timestamps[name];
    if (schema[name] && !schema[newName]) {
      model.renameAttribute(name, newName);
    }
  }
}
...
class Bone {
  ...
  /**
   * Generate `schema`, `schemaMap`, and attributes from schema information.
   * @private
  ...
  static init(columns = []) {
    ...
    const { underscored } = this.defineConfig;
    for (const columnInfo of columns) {
      const { columnName } = columnInfo;
      const name = underscored || columnName == '_id'
        ? columnName
        : camelCase(columnName);

      schema[name] = columnInfo;
      schemaMap[columnName] = name;
    }
    ...
    if (!underscored) renameTimestamps(this);
    this.synchronized = Object.keys(this.diff).length === 0;
  }
   ...
   */
  static get columns() {
    return Object.values(this.schema).map(({ columnName }) => columnName);
  }
  ...
  static unalias(name) {
    if (name in this.schema) {
      return this.schema[name].columnName;
    } else {
      return name;
    }
  }
  ...
  static renameAttribute(originalName, newName) {
    ...
    if (this.schema.hasOwnProperty(originalName)) {
      const info = this.schema[originalName];
      this.schema[newName] = info;
      this.schemaMap[info.columnName] = newName;
  ...
  static reflectType(dataType) {
    if (dataType === 'INTEGER' || dataType.startsWith('int') || dataType.startsWith('decimal')) {
      return Number;
    }
  ...
  static relate(name, opts) {
    ...
    if (Model.schema[deletedAt] && !opts.where) {
      ...
      for (let prop in row) {
        ...
        if (qualifier == '') {
          ...
          for (const columnName in data) {
            const name = Model.schemaMap[columnName];
            if (name) obj[name] = data[columnName];
  ...
  static instantiate(row) {
    ...
    for (const name in this.schema) {
      const { columnName, type } = this.schema[name];
  ...
  static define(name, definitions, { tableName } = {}) {
    if (!tableName) {
      throw new Error('unable to define model without table name');
    }
  ...
  static get diff() {
    ...
    for (const name in definitions) {
      ...
      const { dataType, allowNull, defaultValue } = definition;
      const columnInfo = schema[name];

      if (!columnInfo ||
          dataType.type !== columnInfo.dataType ||
  ...
  static async sync() {
    const { diff, schema, physicTable: table } = this;

    if (Object.keys(schema).length > 0) {
      for (const name in diff) {
        const definition = diff[name];
        const { columnName } = definition;
        if (schema[name]) {
          await this.driver.changeColumn(table, columnName, definition);
        } else {
          await this.driver.addColumn(table, columnName, definition);
        }
      }
    } else {
      await this.driver.createTable(table, diff);
    }

    const schemaInfo = await this.driver.querySchemaInfo('leoric', [ table ]);
    this.init(schemaInfo[table]);
  }
}

Object.assign(Bone, {
  timestamps: [ 'createdAt', 'updatedAt', 'deletedAt' ].reduce((result, name) => {
    result[name] = name;
    return result;
  }, {}),
});
