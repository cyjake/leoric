...
const { LeoricValidateError, executeValidator } = require('./validator');
  ...
  _validateAttribute(attribute, value) {
    ...
    for (const key in validate) {
      if (validate.hasOwnProperty(key)) {
        const executor = validate[key];
        if (typeof executor === 'function') {
          // custom validator
          try {
            // make sure this[name] callable and not return undefined or previous value
            this.raw[name] = value;
            const execRes = executor.call(this, value);
            if (execRes === false) throw new LeoricValidateError(`Validation on ${name} failed`);
          } catch (err) {
            // revert value
            this.raw[name] = this.rawInitial[name];
            throw err;
          }
          // revert value
          this.raw[name] = this.rawInitial[name];
        } else {
          executeValidator(this, key, name, executor, value);
        }
      }
    }
  }
