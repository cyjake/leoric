...
class Bone {
  ...
  previousChanged(name) {
    if (name != null) {
      if (this.rawUnset.has(name) || this.rawPrevious[name] === undefined) return false;
      const value = this.attribute(name);
    ...
    for (const attrKey of Object.keys(this.constructor.attributes)) {
      ...
      const value = this.attribute(attrKey); // rawValue == null ? null : rawValue;
      const valueWas = this.rawPrevious[attrKey] == null ? null : this.rawPrevious[attrKey];
      if (!util.isDeepStrictEqual(value, valueWas)) result.push(attrKey);
    }
  ...
  changes(name) {
    if (name != null) {
      if (this.rawUnset.has(name) || this.rawSaved[name] === undefined) return {};
      const value = this.attribute(name);
      const valueWas = this.attributeWas(name);
      if (util.isDeepStrictEqual(value, valueWas)) return {};
      return { [name]: [ valueWas, value ] };
    ...
    const result = {};
    for (const attrKey of Object.keys(this.constructor.attributes)) {
      if (this.rawUnset.has(name) || this.rawSaved[attrKey] === undefined) continue;
      const value = this.attribute(attrKey);
  ...
  /**
   * Sync changes made in {@link Bone.raw} back to {@link Bone.rawSaved}. Mostly used after the changes are persisted to database, to make {@link Bone.attributeChanged} function properly.
   * @private
   */
  syncRaw(changes) {
  ...
  async reload() {
    ...
    if (instance) {
      ...
      Object.assign(this.rawSaved, instance.rawSaved);
      Object.assign(this.rawPrevious, instance.rawPrevious);
    }
  }
  ...
  static instantiate(row) {
    ...
    const { raw, rawSaved, rawUnset } = instance;
    ...
    for (const name in attributes) {
      ...
      if (columnName in row) {
        ...
        rawSaved[name] = driver.cast(row[columnName], jsType);
      } else {
        rawUnset.add(name);
      }
    }
