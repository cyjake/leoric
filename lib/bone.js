...
const { executeValidator } = require('./validator');
...
class Bone {
  ...
  attribute(...args) {
    ...
    if (args.length > 1) {
      // execute validators
      this._validateAttribute(attributes[name], value);
      this.raw[name] = value;
  ...
  _validateAttribute(attribute, value) {
    const { validate, name } = attribute;
    if (!validate) return;
    for (const key in validate) {
      if (validate.hasOwnProperty(key)) executeValidator(this, key, name, validate[key], value);
    }
  }
  ...
  /**
   * @deprecated {attributeChanged} will be deprecated, RECOMMEND to use {@link Bone.changed}
