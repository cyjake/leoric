...
function renameTimestamps(model) {
  ...
  for (const name in timestamps) {
    ...
    if (attributes[name] && !attributes[newName]) {
...
function compareDefinition(definition, columnInfo) {
  ...
  for (const prop of [ 'dataType', 'defaultValue', 'allowNull' ]) {
    const def = definition[prop];
    const info = columnInfo[prop];

    if (prop === 'dataType') {
      return def && info && (def === info || def.type === info);
    }
...
function compare(attributes, columns) {
  ...
  for (const name in attributes) {
    ...
    if (!compareDefinition(definition, columnMap[columnName])) {
      diff[name] = {
        exists: columnMap.hasOwnProperty(columnName),
        ...definition,
      };
    }
  }
...
class Bone {
  ...
  update() {
    const data = {};
    const { attributes, primaryKey, shardingKey } = this.constructor;
  ...
  static initAttributes(columns) {
    // Bone.define('Note', { title, body })
    // Note.init({ title, body })
    if (this.attributes) return;

    const attributes = {};
    const { underscored } = this.options.define;
    ...
    for (const columnInfo of columns) {
      ...
      attributes[name] = { ...columnInfo, type: this.reflectType(dataType) };
    }

    Object.defineProperties(this, looseReadonly({ attributes }));
  }
   ...
   */
  static load(columns = []) {
    // models in Active Record style
    if (!this.attributes) this.initAttributes(columns);
  ...
  static get tableName() {
    if (!this.table) {
      Object.defineProperties(this, looseReadonly({
        table: snakeCase(pluralize(this.name)),
      }));
    }

    if (!this.aliasName) {
      Object.defineProperties(this, looseReadonly({
        aliasName: camelCase(pluralize(this.name)),
      }));
    }
  ...
  static define(name, attributes, opts = {}) {
    opts = { table: pluralize(snakeCase(name)), ...opts };
    const Model = class extends this {};
    Model.init(attributes, opts);
    return Model;
  }

  static init(attributes, { table } = {}) {
    const result = {};
  ...
  static async sync() {
    const { driver, tableName: table } = this;
    ...
    const diff = compare(attributes, columns);

    if (Object.keys(columns).length === 0) {
      await driver.createTable(table, diff);
    } else {
      await driver.alterTable(table, diff);
    }
