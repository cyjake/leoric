...
/**
 * The Bone to extend models from.
 * @module
 */
...
/**
 * The base class that provides Object-relational mapping. This class is never intended to be used directly. We need to extend sub-classes, called models, from it.
 * @alias Bone
 * @example
 * class Post extends Bone {}
 * async function() {
 *   const posts = await Post.all
 *   for (const post of posts) handle(post)
 * }
 */
class Bone {
  /**
   * Create an instance of Bone.
   * @param {Object} attributes - initial attributes
   * @example
   * class Post extends Bone {}
   * new Post()
   * new Post({ title: 'King Leoric' })
   */
  ...
  /**
   * Get or set attribute value by name. If the attribute isn't selected when queried from database, an error will be thrown when accessing it. This is the underlying method of attribute getter/setters:
   *
   ...
   * @returns {*} - returns the value of attribute if value is undefined.
   * @example
   * .attribute('title')                 // get the value of title
   * .attribute('title', 'King Leoric')  // set the value of title to 'King Leoric'
   */
  ...
  /**
   * Gets called when `console.log(instance)` is called, hence this method can be used to override console.log behavior.
   * @returns {String}
   */
  inspect() {
  ...
  /**
   * Gets called when `JSON.stringify(instance)` is called, hence this method can be used to override JSON.stringify behavior.
   * @returns {Object}
   */
  toJSON() {
    const obj = {}
  ...
  /**
   * @returns {number} - number of affectedRows
   */
  upsert() {
    const data = {}
  ...
  /**
   * Override attribute metadata such as `type`:
   *
