...
class Bone {
  ...
  toJSON(exceptKeys = []) {
    ...
    for (const key of names) {
      if (this.#rawUnset.has(key) || key === 'isNewRecord' || exceptKeys.includes(key)) continue;
      if (typeof this[key] !== 'function') {
        const value = this[key];
        if (value != null) {
          obj[key] = typeof value.toJSON === 'function' ? value.toJSON(exceptKeys) : value;
        }
      }
    }
  ...
  static update(conditions, values = {}, queryOption = {}) {
    ...
    if (!queryOption || queryOption.validate !== false) {
      // validate
      const instance = new this();
      instance._validateAttributes(data);
    }
