...
class Bone {
  ...
  static init(columns = []) {
     ...
     *         columnDefault, // the column default
  ...
  static define(name, definitions, defineConfig) {
  ...
  static get diff() {
    ...
    for (const name in definitions) {
      ...
      const { type, allowNull, defaultValue } = definition;
      const columnInfo = schema[name];

      if (!columnInfo ||
          type.toSqlString() !== columnInfo.dataType.toUpperCase() ||
  ...
  static async sync() {
    const { diff, definitions, schema, physicTable: table } = this;

    if (Object.keys(schema).length === 0) {
      return await this.driver.createTable(table, definitions);
    }
    return await this.driver.alterTable(diff);
  }
}
