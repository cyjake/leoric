...
class Bone {
  ...
  toJSON() {
    ...
    for (const key of names) {
      if (this.#rawUnset.has(key) || key === 'isNewRecord') continue;
  ...
  /**
   * @private avoid override by user and not return {@link Spell}
   * Start a find query by creating and returning an instance of {@link Spell}. The `conditions` and `values` are handed over to {@link spell#$where}.
   * @param {string|Object} conditions
   * @param {...*} values
   * @return {Spell}
  ...
  static create(values, opts = {}) {
    ...
    if (opts.validate !== false) {
      instance._validateAttributes(values); // call instance._validateAttributes manually to validate the raw value
    }
  ...
  static update(conditions, values = {}, queryOption = {}) {
    ...
    const spell = new Spell(this, queryOption).$where(conditions).$update(values);
    return spell.later(result => {
      return result.affectedRows;
    });
  }
