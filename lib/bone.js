...
class Bone {
  ...
  upsert() {
    ...
    const { schema, logger, pool, primaryKey, table } = Model
    ...
    const spell = new Spell(Model, sql => {
      ...
      return new Promise((resolve, reject) => {
        ...
        pool.query(sql, (err, results) => {
          ...
          this.syncRaw()
          logger.info(`[Dal] [${table}] [update] [${new Date() - startedAt}] [${affectedRows}]`)
  ...
  create() {
    const Model = this.constructor
    const { logger, pool, primaryKey, schema, table } = Model
    const data = {}
    ...
    const spell = new Spell(Model, sql => {
      ...
      return new Promise((resolve, reject) => {
        ...
        pool.query(sql, (err, results, fields) => {
          ...
          logger.info(`[Dal] [${table}] [create] [${new Date() - startedAt}] [1]`)
