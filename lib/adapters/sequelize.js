...
module.exports = Bone => {
  return class Spine extends Bone {
    ...
    static async count(options = {}, queryOptions = {}) {
      const { where, col, group } = options;
      let spell = super.find(where);
      if (Array.isArray(group)) spell.$group(...group);
      if (queryOptions.paranoid === false || options.paranoid === false) spell = spell.unscoped;
    ...
    static findAll(options = {}, queryOptions = {}) {
      let spell = this.find();
    ...
    static async findAndCountAll(options, queryOptions = {}) {
      let spell = this.find();
      translateOptions(spell, options);
      if (queryOptions.paranoid === false || options.paranoid === false) spell = spell.unscoped;
    ...
    static async findByPk(value, queryOptions = {}) {
      let spell = super.findOne({ [this.primaryKey]: value });
      if (queryOptions.paranoid === false) spell = spell.unscoped;
      return await spell;
    }
    ...
    static findOne(options, queryOptions = {}) {
      // findOne(null)
      ...
      if (queryOptions.paranoid === false || (options && options.paranoid === false)) spell = spell.unscoped;
      return spell;
    }
