...
module.exports = Bone => {
  return class Spine extends Bone {
    ...
    static async destroy(options = {}, runOpt) {
      const { where } = options;
      return await this.remove(where || {}, { ...runOpt, hooks: false });
    }
    ...
    static update(values, options = {}, runOpt) {
      const { where } = options;
      const whereConditions = where || {};
      return super.update(whereConditions, values, { ...runOpt, hooks: false });
    }

    static upsert(values, options = {}) {
      const instance = new this(values);
      return instance.upsert({ hooks: false });
    }
     ...
     * @param {*} fun
     */
    static addHook(hookName, fnNameOrFun, funRes) {
      if (!hookName || (!fnNameOrFun && !fun)) return;
      let func = funRes;
      if (typeof fnNameOrFun === 'function') func = fnNameOrFun;
      setupSingleHook(this, hookName, func);
    }
