...
module.exports = Bone => {
  return class Spine extends Bone {
    ...
    static async max(attribute, options = {}) {
      let spell = super._find(options.where).$maximum(attribute);
      if (options.paranoid === false) spell = spell.unparanoid;
      return await spell;
    }

    static async min(attribute, options = {}) {
      let spell = super._find(options.where).$minimum(attribute);
      if (options.paranoid === false) spell = spell.unparanoid;
      return await spell;
    }
    ...
    // TODO or TRUNCATE TABLE tableName ?
    static truncate(options) {
      const opt = options || {};
      return this.remove({}, true, opt);
    }
