...
module.exports = Bone => {
  return class Spine extends Bone {
    ...
    static aggregate(name, func, options = {}) {
      ...
      const { where } = options;
      let spell = this._find(where)[`$${func}`](name);
    ...
    static async destroy(options = {}) {
      ...
      if (individualHooks) {
        let findSpell = this._find(where);
        if (paranoid === false) findSpell = findSpell.unscoped;
    ...
    // proxy to class.update({ individualHooks=false }) see https://github.com/sequelize/sequelize/blob/4063c2ab627ad57919d5b45cc7755f077a69fa5e/lib/model.js#L2895  before(after)BulkUpdate
    static bulkUpdate(values, options = {}) {
      const { where, paranoid = false, validate } = options;
      const whereConditions = where || {};
      const spell = super.update(whereConditions, values, { validate, hooks: false });
