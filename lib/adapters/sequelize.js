...
module.exports = Bone => {
  return class Spine extends Bone {
    ...
    static async update(values, options = {}) {
      const { where, paranoid = false, validate, individualHooks } = options;
      if (individualHooks) {
        let findSpell = this.find(where);
        if (!paranoid) {
          findSpell = findSpell.unscoped;
        }
        const instances = await findSpell;
        if (instances.length) {
          return await Promise.all(instances.map((instance) => instance.update(options, { validate })));
        }
      } else {
