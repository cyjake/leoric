...
module.exports = Bone => {
  return class Spine extends Bone {
    ...
    static init(attributes, opts = {}, descriptors = {}) {
      ...
      const overrides = compose((result) => {
        Object.keys(setterMethods).map(key => {
          if (!result[key]) result[key] = {};
          result[key].set = setterMethods[key];
        });
        return result;
      }, (result) => {
        Object.keys(getterMethods).map(key => {
          if (!result[key]) result[key] = {};
          result[key].get = getterMethods[key];
        });
        return result;
      })({});
      Object.defineProperties(this.prototype, looseReadonly(overrides));
    }
