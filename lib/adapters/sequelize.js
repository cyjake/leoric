...
module.exports = Bone => {
  return class Spine extends Bone {
    ...
    // store getterMethods' name
    static customGetters = new Set();
    ...
    toJSON() {
      ...
      for (const name of this.constructor.customGetters) {
        const value = this[name];
        if (value != null) {
          result[name] = typeof value.toJSON === 'function' ? value.toJSON() : value;
        }
      }
    ...
    toObject() {
      ...
      for (const name of this.constructor.customGetters) {
        const value = this[name];
        result[name] = value != null && typeof value.toObject === 'function'
          ? value.toObject()
          : value;
      }
      return result;
