 ...
 * }
 */
function formatArgs(isInstance, fnName, args, target, sequelize) {
  let useHooks = true;
  let opts;
  let argsRes;
  switch (fnName) {
    ...
    case 'update': case 'remove': {
      if (isInstance || sequelize) {
        // instance.update(opts) instance.remove(forceUpdate)
        // class.update(values = {}, options)
        opts = args[1];
        if (opts && opts.hooks === false) useHooks = false;
        argsRes = [ target, ...args ];
        break;
...
// bulkDestroy equals to class.destroy() witch individualHooks=false
// bulkUpdate equals to class.update witch individualHooks=false
const hookableMethods = [ 'update', 'create', 'destroy', 'upsert', 'remove', 'save', 'bulkDestroy', 'bulkUpdate' ];
const hookType = {
  BEFORE: 'before',
  AFTER: 'after',
};
...
function addHook(target, hookName, func) {
  ...
  if (hookableMethods.includes(method)) {
    ...
    target.prototype[method] = async function() {
      const { useHooks, args } = formatArgs(true, method, arguments, this, sequelize);
    ...
    target[method] = async function() {
      const { useHooks, args } = formatArgs(false, method, arguments, this, sequelize);
      if (useHooks && type === hookType.BEFORE) {
        await func.apply(this, args);
      }
