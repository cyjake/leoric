...
function parseExprList(str, ...values) {
  ...
  function keyword() {
    ...
    if (value == 'not' && /^\s+(?:between|in|like)/i.test(str.slice(i))) {
      space()
      value += ` ${keyword()}`
    }
    else if (value == 'is' && /^\s+not/i.test(str.slice(i))) {
      space()
      keyword()
      space()
      value += ' not'
    }
    return OPERATOR_ALIAS_MAP[value] || value
  }
  ...
  function operator(t) {
    ...
    if (name == 'between' || name == 'not between') {
      ...
      if (operand.type == 'op' && !isLower && precedes(name, operand.name) <= 0) {
        const { args } = operand
        operand.args = [
      ...
      else if (operand.type !== 'op' && t.type == 'op' && precedes(name, t.name) < 0) {
        t.args[1] = { type: 'op', name, args: [ t.args[1], operand ] }
        return t
      }
      else {
