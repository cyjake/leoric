...
function formatInsert(spell) {
  ...
  if (shardingKey && sets[shardingColumn] == null) {
...
function formatUpdate(spell) {
  ...
  if (shardingKey) {
    if (sets.hasOwnProperty(shardingColumn) && sets[shardingColumn] == null) {
      throw new Error(`Sharding key ${Model.table}.${shardingKey} cannot be NULL`)
    }
    if (!whereConditions.some(condition => findExpr(condition, { type: 'id', value: shardingColumn }))) {
      throw new Error(`Sharding key ${Model.table}.${shardingKey} is required.`)
    }
  }
