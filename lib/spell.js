...
const { HINT_TYPE, Hint, buildHint } = require('./hint');
...
class Spell {
  ...
  constructor(Model, opts = {}) {
    ...
    if (opts.hints && opts.hints.length) {
      hints.push(...opts.hints.map(hint => buildHint(hint)));
    }

    if (opts.hint) {
      hints.push(buildHint(opts.hint));
    }

    let uniqueHints = [];
    // unique all hints
    hints.reduce(((arr, val) => {
