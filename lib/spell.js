...
/**
 * The spell to generate SQL strings
 * @module
 */
...
/**
 * Construct on conditions as ast from associations, e.g.:
 *
...
/**
 * Find relation by certain criteria, e.g. findRelation({ Model })
...
/**
 * Spell is the query builder of Leoric which provides several important features that made a powerful querying interface possible.
 * - Deferred
 * - Method chaining
...
class Spell {
   ...
   *
   * The `values` is only necessary when using templated string conditions, which for security reasons is highly recommended.
   *
   * @param {string|Object} conditions
   * @param {...*}          values
   * @returns {Spell}
  ...
  /**
   * Set GROUP BY attributes. To simplify the query building, we can just:
   *
   *     .group('YEAR(createdAt) AS year')
   *     // instead of
   *     .select('YEAR(createdAt)) AS year').group('year')
   *
   * More than one group attributes can be passed:
   *
  ...
  /**
   * Turn the query into batch mode, returns an async interator which can then be consumed with an async loop or the cutting edge `for await`.
   * @example
