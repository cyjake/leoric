...
/**
 * Walk through an ast with returned tokens preferred over the originals, which is convenient to update the ast. For example, to update the identifiers in the ast with their qualifiers set to `posts`:
 *
 *     copyExpr(ast, ({ type, value }) => {
 *         if (type == 'id') return { type, qualifiers: ['posts'], value }
 *     })
 *
 * Please be noted that the ast is still overriden in this case.
 * @param {Object}   ast
 * @param {function} fn
 */
